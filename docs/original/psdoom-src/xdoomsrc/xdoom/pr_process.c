...
mobj_t *P_SpawnMapThing(mapthing_ext_t *mthing, boolean is_pid_mobj); /* from p_mobj.c */
...
walisser  8968   4.2 14.0   465360 147056  ??  S     8:07PM  47:30.15 /Applications/Safari.app/Contents/MacOS/Safari -psn_0_8257537
walisser   445   1.3  0.3    76024   3168  ??  S    Wed09AM  72:29.35 /Applications/Utilities/CPU Monitor.app/Contents/MacOS/CPU Monitor -psn_0_786433
walisser   448   1.2  1.2    88856  12992  ??  S    Wed09AM  10:13.35 /Applications/Utilities/Terminal.app/Contents/MacOS/Terminal -psn_0_1310721
  */
  f = popen("ps -auxww", "r");
  ...
  while (fgets(buf, 255, f)) {
    ...
    sscanf(buf, "%s %d %*f %*f %*d %*d %*s %s %*s %*s %s",
          username, &pid,              tty,            namebuf);
    if (tty[0]=='?')  /* if there is no tty, then it is a daemon */
      demon = true;
    else

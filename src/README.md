...
| `platform/` | Backend-neutral process layer — `proc_types.h` (the `psd_proc_t` record), `proc_backend.{h,c}` (the enumerate/uid/renice/kill vtable + active-backend selection), and `proc_fake.{h,c}` (a scriptable in-memory backend for tests). |
